 ...
 * Complete breakdown tracking, rendering, sharing, and download operations
 */
...
export default function PayslipScreen() {
  ...
  const loadPayslips = async () => {
    ...
    try {
      ...
      console.error("Error loading payslips ledger:", error);
      Alert.alert("Network Failure", "Failed to retrieve compiled payslip archives.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPayslip = async (payslip: PayslipRecord) => {
    ...
    try {
      ...
      Alert.alert("Success", `Payslip downloaded to local disk paths: ${filePath}`);
    } catch (error) {
      console.error("PDF engine crash error details:", error);
      Alert.alert("Error", "PDF generator engine encountered a serialization block.");
    } finally {
      setDownloading(null);
    }
  };
  ...
  const handleSharePayslip = async (payslip: PayslipRecord) => {
    ...
    try {
      ...
      console.error("System native sheet capture error:", error);
      Alert.alert("Error", "Could not capture device share sheets.");
    } finally {
      setSharing(null);
    }
  };
  ...
  const renderPayslipItem = ({ item, index }: { item: PayslipRecord; index: number }) => {
    ...
    let formattedMonth = "Salary Statement";
    try {
      if (item.month) {
        formattedMonth = new Date(item.month).toLocaleDateString("en-US", { month: "long", year: "numeric" });
      }
    } catch (_) {}
      ...
      <Animated.View entering={FadeInDown.delay(index * 40).springify()}>
        <PremiumCard style={{ marginBottom: THEME.spacing.md }}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: THEME.spacing.md, borderBottomWidth: 1, borderBottomColor: themeStyles.border, paddingBottom: THEME.spacing.sm }}>
            <View>
              <Text style={{ fontSize: 16, fontWeight: "700", color: themeStyles.text.primary }}>📄 {formattedMonth}</Text>
              {paidDate && (
            ...
            <View style={{ backgroundColor: THEME.colors.successLight, paddingHorizontal: 10, paddingVertical: 4, borderRadius: THEME.borderRadius.sm }}>
              <Text style={{ color: THEME.colors.success, fontSize: 10, fontWeight: "700", textTransform: "uppercase" }}>Disbursed</Text>
            </View>
          </View>

          {/* Core breakdown table container */}
          <View style={{ gap: THEME.spacing.xs, marginBottom: THEME.spacing.md }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <Text style={{ fontSize: 13, color: themeStyles.text.secondary }}>Gross Income</Text>
              <Text style={{ fontSize: 13, fontWeight: "600", color: themeStyles.text.primary }}>₹{grossSalary.toLocaleString("en-IN")}</Text>
            </View>
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <Text style={{ fontSize: 13, color: themeStyles.text.secondary }}>Deductions & Tax withholding</Text>
              <Text style={{ fontSize: 13, fontWeight: "600", color: THEME.colors.danger }}>- ₹{totalDeductions.toLocaleString("en-IN")}</Text>
            </View>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: themeStyles.border, paddingTop: THEME.spacing.sm, marginTop: THEME.spacing.xs }}>
              <Text style={{ fontSize: 14, fontWeight: "700", color: themeStyles.text.primary }}>Net Cash Payout</Text>
              <Text style={{ fontSize: 16, fontWeight: "800", color: THEME.colors.success }}>₹{netSalary.toLocaleString("en-IN")}</Text>
            </View>
          </View>

          {/* Shared Action Toolbar Row */}
          <View style={{ flexDirection: "row", gap: THEME.spacing.sm, borderTopWidth: 1, borderTopColor: themeStyles.border, paddingTop: THEME.spacing.md }}>
            <TouchableOpacity onPress={() => handlePreviewPayslip(item)} style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 10, borderRadius: THEME.borderRadius.md, backgroundColor: isDark ? THEME.colors.dark.backgroundTertiary : "#f1f5f9", borderWidth: 1, borderColor: themeStyles.border, gap: 4 }}>
            ...
            <TouchableOpacity disabled={downloading === item.id} onPress={() => handleDownloadPayslip(item)} style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 10, borderRadius: THEME.borderRadius.md, backgroundColor: THEME.colors.primary, gap: 4 }}>
              {downloading === item.id ? <ActivityIndicator size="small" color="#fff" /> : <><MaterialCommunityIcons name="download" size={16} color="#fff" /><Text style={{ fontSize: 12, fontWeight: "600", color: "#fff" }}>Download</Text></>}
            </TouchableOpacity>

            <TouchableOpacity disabled={sharing === item.id} onPress={() => handleSharePayslip(item)} style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 10, borderRadius: THEME.borderRadius.md, backgroundColor: "#8B5CF6", gap: 4 }}>
              {sharing === item.id ? <ActivityIndicator size="small" color="#fff" /> : <><MaterialCommunityIcons name="share-variant" size={16} color="#fff" /><Text style={{ fontSize: 12, fontWeight: "600", color: "#fff" }}>Share</Text></>}
            </TouchableOpacity>
          </View>
        </PremiumCard>
      </Animated.View>
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <View style={{ flex: 1, justifyContent: "center", alignItems: "center", marginTop: 100 }}>
            ...
            <Text style={{ fontSize: 15, fontWeight: "600", color: themeStyles.text.secondary, textAlign: "center" }}>No generated statements found</Text>
            <Text style={{ fontSize: 12, color: themeStyles.text.tertiary, textAlign: "center", marginTop: 4, paddingHorizontal: THEME.spacing.xl }}>Payslips show up here once generated by an administrator for the current salary period.</Text>
          </View>
        )}
